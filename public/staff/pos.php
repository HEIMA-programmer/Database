...
async function loadOrderDetail(orderId) {
    ...
    try {
        ...
        const response = await fetch('api_get_order_detail.php', {
            method: 'POST',
            body: formData
        });
        ...
        html += '<div class="row">';
        html += '<div class="col-6"><strong class="text-muted">Customer:</strong> <span class="text-white">' + escapeHtml(result.info.customer) + '</span></div>';
        html += '<div class="col-6"><strong class="text-muted">Date:</strong> <span class="text-white">' + escapeHtml(result.info.date) + '</span></div>';
        html += '</div>';
        html += '<div class="row mt-2">';
        html += '<div class="col-6"><strong class="text-muted">Status:</strong> <span class="badge bg-' + (result.info.status == 'Completed' ? 'success' : 'info') + '">' + escapeHtml(result.info.status) + '</span></div>';
        html += '<div class="col-6"><strong class="text-muted">Total:</strong> <span class="text-warning fw-bold">¥' + parseFloat(result.info.total).toFixed(2) + '</span></div>';
        html += '</div></div>';

        html += '<h6 class="text-warning mt-3 mb-2"><i class="fa-solid fa-compact-disc me-2"></i>Items (' + result.items.length + ')</h6>';
